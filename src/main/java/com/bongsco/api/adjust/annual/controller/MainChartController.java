...
import com.bongsco.api.adjust.annual.dto.response.EmploymentTypeDistributionResponse;
import com.bongsco.api.adjust.annual.dto.response.GradeDistributionResponse;
import com.bongsco.api.adjust.annual.dto.response.HeadCountTrendResponse;
import com.bongsco.api.adjust.annual.dto.response.TenureDistributionResponse;
import com.bongsco.api.adjust.annual.dto.response.TotalSalaryTrendResponse;
...
public class MainChartController {
    ...
    @Operation(summary = "총 급여 상승률", description = "가장 최근 ANNUAL 차수 기준 전체 급여 총합과 전년 대비 상승률 반환")
    public ResponseEntity<TotalSalaryTrendResponse> getTotalSalaryTrend() {
        ...
        if (currentTotal == null)
            currentTotal = 0L;
        if (prevTotal == null || prevTotal == 0L) {
            return ResponseEntity.ok(
                new TotalSalaryTrendResponse(currentTotal, 0L, 0.0)
            );
        }

        double increaseRate = Math.round(
            ((currentTotal - prevTotal) * 100.0 / prevTotal) * 10.0
        ) / 10.0;

        return ResponseEntity.ok(
            new TotalSalaryTrendResponse(currentTotal, prevTotal, increaseRate)
        );
    }
}
