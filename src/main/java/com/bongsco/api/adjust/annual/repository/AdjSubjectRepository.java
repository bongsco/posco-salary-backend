...
public interface AdjSubjectRepository extends JpaRepository<AdjSubject, Long> {
    ...
    @Query("SELECT asj FROM AdjSubject asj WHERE asj.adjInfo.id = :adjInfoId "
        + "AND (asj.employee.empNum LIKE %:searchKey% OR asj.employee.name LIKE %:searchKey%)")
    List<AdjSubject> findByAdjInfo_IdAndSearchKey(Long adjInfoId, String searchKey);
    ...
    @Query(
        "SELECT new com.bongsco.api.annual.dto.AdjSubjectSalaryDto(asj.id, asj.employee.id, asj.stdSalary, asj.finalStdSalary, asj.paybandUse, asj.grade.id, asj.rank.rankName, pc.upperLimitPrice, pc.grade.gradeName) "
            + "FROM AdjSubject asj "
    ...
    @Query(
        "SELECT new com.bongsco.api.annual.dto.AdjSubjectSalaryDto(asj.id, asj.employee.id, asj.stdSalary, asj.finalStdSalary, asj.paybandUse, asj.grade.id, asj.rank.rankName, pc.lowerLimitPrice, pc.grade.gradeName) "
            + "FROM AdjSubject asj "
            + "JOIN PaybandCriteria pc ON pc.grade.id = asj.grade.id "
            + "WHERE asj.adjInfo.id = :adjInfoId AND pc.adjInfo.id = :adjInfoId AND asj.subjectUse = true "
            + "AND asj.deleted!=true AND pc.deleted!=true AND asj.subjectUse=true")
